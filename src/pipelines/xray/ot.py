"""
Optimal Transport (OT) data valuation for X-ray classification.

Computes per-training-sample OT scores using Sinkhorn optimal transport:
cost = cosine distance, reward = label agreement.
"""
...
def compute_ot_values(train_feats, train_y, val_feats, val_y, args):
    ...
    scores = ot_binary_row_normalized(train_feats, train_y, val_feats, val_y, reg=args.ot_reg)
    return scores.numpy()
